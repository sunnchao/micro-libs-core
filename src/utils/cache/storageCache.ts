...
class WebStorage {
  ...
  constructor(storage: Storage, prefixKey: string, encryption: AesEncryption, timeout: null | number, hasEncrypt: boolean) {
...
export const createStorage = ({ prefixKey = '', storage = sessionStorage, key = cacheCipher.key, iv = cacheCipher.iv, timeout = null, hasEncrypt = true }: Partial<CreateStorageParams> = {}) => {
  ...
  const _storage = new WebStorage(storage, prefixKey, encryption, timeout, hasEncrypt);
